...
#SBATCH --nodelist=node10          # Specific node
#SBATCH --partition=7501                # MPI ranks (for pw/ph/epw etc.)
#SBATCH --ntasks=64
...
# ==== 11.5) Run lambda.x (single-core 권 장 ) ====================
if ((${#LAMBDA_INPUTS[@]})); then
  echo ">>> Running lambda.x jobs (single-core)"
  for inpf in "${LAMBDA_INPUTS[@]}"; do
    if [[ ! -f "$inpf" ]]; then
      echo "[LAMBDA] WARNING: input file '$inpf' not found, skipping"
      continue
    fi
    stem="${inpf%.in}"
    echo "[LAMBDA] ${inpf} -> ${stem}.out"
    srun -n 1 "${LAMBDA_BIN}" -in "${inpf}" > "${stem}.out"
  done
else
  echo ">>> No LAMBDA_INPUTS; skipping lambda.x block"
fi
